...
# CHANGE 2: Initialize db with app BEFORE importing shariah models
db.init_app(app)
...
# CHANGE 3: Import shariah models AFTER db initialization
try:
            ComprehensiveShariahAssessment, 
        ShariahAssessmentAudit, 
        ShariahProductTemplate,
...
def create_default_users():
    ...
    def initialize_enhanced_shariah():
        """Initialize enhanced Shariah system"""
    if not ENHANCED_SHARIAH_AVAILABLE:
        return None, None
    ...
    try:
        ...
        init_shariah_routes(
            database=db,
            shariah_models=shariah_models,
            shariah_scoring_engine=scoring_engine,
            get_current_user_func=get_current_user,
            role_required_func=role_required,
    ...
    print(f"⚠️ Enhanced Shariah not available: {e}")
    ENHANCED_SHARIAH_AVAILABLE = False

# Initialize the enhanced system
enhanced_models, enhanced_engine = initialize_enhanced_shariah()
...
def validate_password_strength(password):
    ...
    # =====================================================
# STEP 4: ADD SAMPLE DATA CREATION FUNCTION
# =====================================================

def create_sample_shariah_templates():
    """Create sample Shariah product templates if they don't exist"""
    
    sample_templates = [
        {
            'product_type': 'murabaha',
            'template_name': 'Standard Murabaha Financing',
            'description': 'Cost-plus sale financing template',
            'required_parameters': [
                'asset_identification', 'cost_price', 'profit_margin', 
                'ownership_transfer', 'payment_terms'
            ],
            'scoring_weights': {
                'fundamental_compliance': 0.45,  # Higher weight for Murabaha
            ...
            'prohibited_elements': [
                'Interest-based penalty', 'Uncertain profit margin',
                'Sale of non-existent assets', 'Riba al-fadl'
            ]
        },
        {
            'product_type': 'mudarabah',
            'template_name': 'Profit-Sharing Investment',
            'description': 'Mudarabah investment template',
            'required_parameters': [
            ...
            'scoring_weights': {
                ...
                'business_activity': 0.25,  # Higher for business screening
                'governance': 0.12,
                'operational': 0.02,
            ...
            'prohibited_elements': [
                'Guaranteed profit', 'Fixed return to mudarib',
                'Penalty for loss', 'Investor interference in management'
            ]
        }
    ]
    ...
    for template_data in sample_templates:
        ...
        existing = ShariahProductTemplate.query.filter_by(
            product_type=template_data['product_type']
        ).first()
        
        if not existing:
            template = ShariahProductTemplate(
                product_type=template_data['product_type'],
    ...
    db.session.commit()
    print("Sample Shariah product templates created!")

    # =====================================================
# STEP 5: UPDATE YOUR EXISTING SHARIAH ROUTES (OPTIONAL)
# =====================================================

# You can keep your existing routes for backward compatibility
# or redirect them to the new comprehensive assessment:

@app.route('/shariah-risk-assessment-legacy', methods=['GET', 'POST'])
@role_required(UserRole.SHARIAH_OFFICER, UserRole.ADMIN)
def shariah_risk_assessment_legacy():
    """
    Legacy route - redirects to comprehensive assessment
    Keep this for backward compatibility
    """
    flash('Redirected to enhanced comprehensive assessment', 'info')
    return redirect(url_for('shariah.comprehensive_assessment'))

    
# Modify your existing database initialization section:
...
if __name__ == '__main__':
    with app.app_context():
        ...
        # Initialize your existing default users
        create_default_users()
        
        # Optional: Create sample Shariah product templates
        create_sample_shariah_templates()
